...
6. **Run the app in development** (API + Vite dev server with proxy):
   ...
   - PWA / Offline install & Smartphone-HTTPS (LAN IP): see [`PWA / Icons (offline install)`](#pwa--icons-offline-install)
...
<a id="pwa--icons-offline-install"></a>
## PWA / Icons (offline install)

The client is configured as a PWA (manifest + service worker) so users can install it and re-open it after refresh or limited offline usage.

### Icons & Favicon

Brand icon source: `client/src/components/common/AppIcon.vue` (`name="trophy"`), adapted to look like a trophy/pokal.

Static icon assets are stored in `client/public/`:

- `client/public/favicon.svg` / `client/public/favicon.ico`
- `client/public/favicon-16x16.png`, `favicon-32x32.png`, `favicon-48x48.png`, `favicon-64x64.png`
- `client/public/apple-touch-icon.png` (180x180)
- `client/public/android-chrome-192x192.png`
- `client/public/icon-512x512.png` (used for PWA install icon)

`client/index.html` links the manifest and favicons (e.g. `rel="manifest"` and `rel="icon"`).

### Web App Manifest

- Manifest file: `client/public/manifest.webmanifest`
- Includes:
  - `icons` (192x192 and 512x512)
  - `screenshots` entries (one for `form_factor: "wide"` and one for non-`wide`) to satisfy Chrome’s install UI checks
  - `id: "/"` (stable app identity; Chrome warns without it)

### Service Worker

- Service worker is generated by `vite-plugin-pwa` in dev/build (`client/vite.config.ts`, `strategies: "generateSW"`).
- Conservative caching:
  - precaches only the app shell + static assets (index + `/assets/**` + icon files)
  - does **not** cache API responses (`/api/*`) and SSE streams (`/api/sse`)

The install/refresh/update flow uses `registerType: "autoUpdate"` and `injectRegister: "auto"`.

### Testing PWA from a Smartphone (LAN IP)

For PWAs, Chrome requires a **secure context (HTTPS)** when using an IP address (not `localhost`).

1. Generate a local dev certificate for your LAN IP (requires `mkcert` installed):
   ```bash
   npm run mkcert:dev -w client
   ```
   This writes certs into `client/.certs/` and prints:
   - where `dev-cert.pem`/`dev-key.pem` are located
   - the `rootCA.pem` you need to trust on the phone

2. Install the printed `rootCA.pem` on your smartphone as a trusted user CA.

3. Start the client dev server over HTTPS:
   ```bash
   npm run dev:https -w client
   ```
   Then open the printed URL on your phone, e.g.:
   - `https://<LAN-IP>:5173/`

If you only test on the same machine, plain `npm run dev` / `http://localhost:5173` works because `localhost` is treated as a secure context.
