...
- **Storybook** (`npm run storybook`): config under `tests/client/storybook/`. Name the primary CSF export **`Default`** when reasonable so URLs and tooling that expect `…--default` keep working; preview wraps the canvas with padding and uses **non-inline** docs stories for reliable Vue rendering. Example: `tests/client/storybook/stories/components/common/CatalogPageHeader.stories.ts`.
...
- **Classes / players:** route `/classes`, `/players`; API `classesApi.ts`, `playersApi.ts` (query `scope=all|own` filters the list only). **Player** dialogs load **all** classes for the class dropdown (`schoolClassOptions` in `playersManagement` store). Server: any authenticated user may PATCH/DELETE any class or player; tournament roster `POST …/members` accepts any catalog player id (`requireTournamentExists` replaces owner checks in `server/src/routes/tournaments/shared.ts`).
