...
- After schema changes: `npm run db:push` (and `db:generate` if needed). On deploy hosts: set `DATABASE_URL`, then `npm run db:deploy` (currently `prisma db push`; no migration history yet).
- Seed: `server/prisma/seed.ts` (demo user, school classes, players, demo tournament). Configured via **server** `package.json` → `prisma.seed`. Re-seeding deletes **all** tournaments, **school classes**, and players for that demo user, then recreates the demo set (so it stays idempotent and avoids FK issues).
...
| Prisma seed | `server/prisma/seed.ts` |
...
| Tournament composables | `client/src/tournament/useTournamentLayoutState.ts`, `useTournamentPhaseStepper.ts` |
